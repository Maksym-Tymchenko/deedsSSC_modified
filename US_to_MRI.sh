...
./linearBCV -F $output_folder/Case${case_num}-MRI_in_US.nii.gz -M $output_folder/Case${case_num}-US.nii.gz -R 1 -O $output_folder/affine
...
python3 ./generate_zero_displacements.py $output_folder $output_folder/Case${case_num}-US.nii.gz

./applyBCV -M $output_folder/Case${case_num}-US-landmarks.nii.gz \
-O $output_folder/zero \
-D $output_folder/Case${case_num}-US-landmarks_linear.nii.gz \
-A $output_folder/affine_matrix.txt

# Perform non linear registration
./deedsBCV -M $output_folder/Case${case_num}-US.nii.gz \
-F $output_folder/Case${case_num}-MRI_in_US.nii.gz \
-O $output_folder/Case${case_num}-deeds \
-S $output_folder/Case${case_num}-US-landmarks.nii.gz \
-A $output_folder/affine_matrix.txt \
-a 1.6 # make a smoother but less accurate registration

# Perform fitting step
./linearBCV -F $output_folder/Case${case_num}-MRI_in_US.nii.gz -M $output_folder/Case${case_num}-deeds_deformed.nii.gz -R 1 -O $output_folder/affine_fit


# Apply fitting step
./applyBCV -M $output_folder/Case${case_num}-deeds_deformed_seg.nii.gz \
-O $output_folder/zero \
-D $output_folder/Case${case_num}-US-landmarks_fitted.nii.gz \
-A $output_folder/affine_fit_matrix.txt


# Calculate mTRE
python3 ./landmarks_centre_mass.py \
--inputnii $output_folder/Case${case_num}-MRI-landmarks.nii.gz \
--movingnii $output_folder/Case${case_num}-US-landmarks_fitted.nii.gz \
