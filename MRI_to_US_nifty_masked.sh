...
#python3 ./transpose.py "$output_folder/affine_matrix.txt"  "$output_folder/affine_matrix_t.txt"
#c3d_affine_tool $output_folder/affine_matrix_t.txt -inv $output_folder/affine_matrix_inv_t.txt
...
-flo $output_folder/clean_MRI.nii.gz \
-res $output_folder/Case${case_num}-MRI_to_US_result.nii.gz \
-aff $output_folder/affine_matrix_fitted_nifty.txt -noSym -rigOnly

# Apply the transformation to the landmarks
reg_resample -ref $output_folder/Case${case_num}-US.nii \
-flo $output_folder/Case${case_num}-deeds_deformed_seg.nii.gz \
-res $output_folder/Case${case_num}-MRI-landmarks_fitted.nii.gz \
-trans $output_folder/affine_matrix_fitted_nifty.txt \
-inter 0

"""
# ./linearBCV -F $output_folder/Case${case_num}-MRI_in_US_linear.nii.gz -M $output_folder/Case${case_num}-deeds_deformed.nii.gz -R 1 -O $output_folder/affine_fit
./linearBCV -F $output_folder/Case${case_num}-US.nii.gz -M $output_folder/Case${case_num}-deeds_deformed.nii.gz -R 1 -O $output_folder/affine_fit


# Apply fitting step
./applyBCV -M $output_folder/Case${case_num}-deeds_deformed_seg.nii.gz \
-O $output_folder/zero \
-D $output_folder/Case${case_num}-MRI-landmarks_fitted.nii.gz \
-A $output_folder/affine_fit_matrix.txt
"""

# Calculate mTRE
python3 ./landmarks_centre_mass.py \
--inputnii $output_folder/Case${case_num}-US-landmarks.nii.gz \
--movingnii $output_folder/Case${case_num}-MRI-landmarks_fitted.nii.gz \
